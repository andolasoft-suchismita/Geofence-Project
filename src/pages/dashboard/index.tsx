...
import { setAttendanceSummary, setMonthlyReport } from '../../redux/slices/dashboardSlice';
...
const Dashboard = () => {
  ...
  const attendanceSummary = useSelector((state: RootState) => state.dashboard.attendanceSummary);
  const monthlyReport = useSelector((state: RootState) => state.dashboard.monthlyReport);
  const company_id = useSelector((state: RootState) => state.authSlice.company_id);
...
const lateComingsToday = attendanceSummary?.late_comings_today || 0;
  ...
  const departmentData = attendanceSummary?.department_wise_attendance
    ? Object.keys(attendanceSummary.department_wise_attendance).map(dept => ({
        department: dept,
        present: attendanceSummary.department_wise_attendance[dept].present || 0,
...
return (
  <div className="bg-gray min-h-screen p-6">
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <Card title="Absentees/Present Today" count={`${absenteesToday}/${presentToday}`} type="absentees" />
      ...
      <div className="mt-6 rounded-md bg-white p-5 shadow">
        ...
        <ResponsiveContainer width="100%" height={300}>
          <BarChart data={monthlyReport}>
            ...
            <Tooltip />
            <Legend />
            <Bar dataKey="days_absent" fill={colorMapping.days_absent} name="Days Absent" />
            <Bar dataKey="half_days" fill={colorMapping.half_days} name="Half Days" />
          </BarChart>
        </ResponsiveContainer>
      </div>
...
//       <Card title="Absentees/Present Today" count= {`${totalAbsent}/${totalPresent}`}  type="absentees" />

...
const DropdownUser = () => {
  const [dropdownOpen, setDropdownOpen] = useState(false);
  const [profile, setProfile] = useState({ name: '', designation: '', image: '' });
  ...
  return (
    <div className="relative">
      ...
      >
        ...
        <span className="h-12 w-12 rounded-full">
          <img src={profile.image} alt="User" className="h-full w-full rounded-full" />
        </span>
      </Link>
      ...
      >
       <ul className="flex flex-col gap-5 border-b border-stroke px-6 py-7.5 dark:border-strokedark">
  <li>
    <Link to="/profile" className="flex items-center gap-3.5 text-sm font-medium hover:text-primary lg:text-base">
      My Profile
    </Link>
  </li>
  {/* <li>
    <Link to="/companysettings" className="flex items-center gap-3.5 text-sm font-medium hover:text-primary lg:text-base">
      Company Setting
        ...
        <button
          onClick={handleLogout}
          className="block w-full py-4 text-center text-sm font-medium hover:text-red-600"
        >
          Logout
        </button>
      </div>

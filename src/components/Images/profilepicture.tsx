    ...
    >
      {profilePic ? (
        <img
          src={profilePic}
          alt="Profile"
          className="border-gray-300 h-24 w-24 rounded-full border-2 object-cover"
        />
      ) : (
        <div className="bg-gray-200 border-gray-300 flex h-24 w-24 items-center justify-center rounded-full border-2">
      ...
      {isDetailsView && isHovered && profilePic && (
        <button
          className="absolute inset-0 flex items-center justify-center rounded-full bg-black bg-opacity-50 text-white transition-opacity duration-200"
